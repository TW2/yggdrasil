    ...
    public void updateDisplayWithOffset(int offset, Time from, Time to){
        if(hasFilePath == true){
            ...
            if(ffss.hasVideo()){
                ...
                horizontalWaveBar.setMaximum(Time.getFrame(Time.substract(from, to), 30d));
            }
    ...
    public Time getTimeOfStartArea(boolean useFPS) {
        if(wp.startArea != null){
            if(useFPS == true){
                ...
                return Time.getTimeFromSamples(ffss.getSampleRate(), samplesPerPixel, wp.startArea.x);
            }
        }
        return Time.create(0L);
    }
    ...
    public Time getTimeOfStopArea(boolean useFPS) {
        if(wp.startArea != null){
            if(useFPS == true){
                ...
                return Time.getTimeFromSamples(ffss.getSampleRate(), samplesPerPixel, wp.stopArea.x);
            }
        }
        return Time.create(0L);
    }
...
//        return Time.getTimeFromFrame(frames, ffss.getFps());
    ...
    public double getSamplesFromPixels(int pixels){        
        // nbSamples = pixels * samplesPerPixel
        return pixels * samplesPerPixel;
    }
    
    public int getPixelsFromSamples(int samples){
        // pixels = nbSamples / samplesPerPixel
        return (int)Math.round(samples / samplesPerPixel);
    }
