 ...
 */
package org.wingate.ygg.drawing.bspline;
...
public class BSplineCurve extends Curve {
    ...
    public BSplineCurve(BezierCurve bezierCurve) {
        ...
        for (int i = 0; i < knots.size(); i++) {
            knots.set(i, (double)(i / (degree + 1d)));
        }
    }
    ...
    private BezierCurve extractBezierCurve(int index) {
        List<Double> tempKnots = new ArrayList<>();
        List<Point2D> tempControlPoints = new ArrayList<>();
    ...
    private void updateKnots() {
        knots = new ArrayList<>(getControlPoints().size() + degree + 1);
